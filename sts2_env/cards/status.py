...
from sts2_env.cards.registry import register_after_combat_end_hook, register_after_card_drawn_hook, register_after_map_generated_hook, register_chosen_hook, register_effect, register_gains_block_hook, register_generated_map_hook, register_generated_map_late_hook, register_next_event_hook, register_playability_hook, register_quest_complete_hook, register_rest_site_options_hook, register_self_mutating_damage, register_should_play_hook, register_target_type_hook, register_turn_end_in_hand_hook, register_unknown_room_types_hook
from sts2_env.core.enums import CardId, CardTag, CardType, TargetType, CardRarity, ValueProp, PowerId, RoomType, MapPointType
...
def _deal_osty_damage_single(card: CardInstance, combat: CombatState, target: Creature, fallback: int=0) -> None:
...
def _hp_loss(card: CardInstance, combat: CombatState, amount: int) -> None:
    owner = _owner(card, combat)
    apply_damage(owner, amount, ValueProp.UNBLOCKABLE | ValueProp.UNPOWERED | ValueProp.MOVE, combat, None)
    combat._check_combat_end()

def _deal_self_damage(card: CardInstance, combat: CombatState, amount: int) -> None:
    owner = _owner(card, combat)
    combat.deal_damage(dealer=owner, target=owner, amount=amount, props=ValueProp.UNPOWERED | ValueProp.MOVE)

def _apply_duration_power_from_turn_end_card(card: CardInstance, combat: CombatState, power_id: PowerId, amount: int) -> None:
    owner = _owner(card, combat)
...
def make_wound() -> CardInstance:
    return CardInstance(card_id=CardId.WOUND, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'unplayable'}), instance_id=_get_next_id())

def make_dazed() -> CardInstance:
    return CardInstance(card_id=CardId.DAZED, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'unplayable', 'ethereal'}), instance_id=_get_next_id())
...
def make_slimed() -> CardInstance:
    return CardInstance(card_id=CardId.SLIMED, cost=1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'exhaust'}), effect_vars={'cards': 1}, instance_id=_get_next_id())

def make_burn() -> CardInstance:
    return CardInstance(card_id=CardId.BURN, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'unplayable'}), effect_vars={'damage': BURN_DAMAGE}, instance_id=_get_next_id(), base_damage=2)

def make_void() -> CardInstance:
    return CardInstance(card_id=CardId.VOID, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'unplayable', 'ethereal'}), effect_vars={'energy': 1}, instance_id=_get_next_id())
...
def make_beckon() -> CardInstance:
    return CardInstance(card_id=CardId.BECKON, cost=1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, effect_vars={'hp_loss': 6}, instance_id=_get_next_id())

def make_debris() -> CardInstance:
    return CardInstance(card_id=CardId.DEBRIS, cost=1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())

def make_frantic_escape() -> CardInstance:
    return CardInstance(card_id=CardId.FRANTIC_ESCAPE, cost=1, card_type=CardType.STATUS, target_type=TargetType.SELF, rarity=CardRarity.STATUS, instance_id=_get_next_id())

def make_infection() -> CardInstance:
    return CardInstance(card_id=CardId.INFECTION, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'unplayable'}), effect_vars={'damage': 3}, instance_id=_get_next_id(), base_damage=3)

def make_soot() -> CardInstance:
    return CardInstance(card_id=CardId.SOOT, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'unplayable'}), instance_id=_get_next_id())

def make_toxic() -> CardInstance:
    return CardInstance(card_id=CardId.TOXIC, cost=1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'exhaust'}), effect_vars={'damage': 5}, instance_id=_get_next_id(), base_damage=5)

def make_ascenders_bane() -> CardInstance:
    return CardInstance(card_id=CardId.ASCENDERS_BANE, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'eternal', 'unplayable', 'ethereal'}), instance_id=_get_next_id())

def make_bad_luck() -> CardInstance:
    return CardInstance(card_id=CardId.BAD_LUCK, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'eternal', 'unplayable'}), effect_vars={'hp_loss': 13}, instance_id=_get_next_id())

def make_clumsy() -> CardInstance:
    return CardInstance(card_id=CardId.CLUMSY, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable', 'ethereal'}), instance_id=_get_next_id())

def make_curse_of_the_bell() -> CardInstance:
    return CardInstance(card_id=CardId.CURSE_OF_THE_BELL, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'eternal', 'unplayable'}), instance_id=_get_next_id())

def make_debt() -> CardInstance:
    return CardInstance(card_id=CardId.DEBT, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), effect_vars={'gold': 10}, instance_id=_get_next_id())

def make_decay() -> CardInstance:
    return CardInstance(card_id=CardId.DECAY, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), effect_vars={'damage': DECAY_DAMAGE}, instance_id=_get_next_id(), base_damage=2)

def make_doubt() -> CardInstance:
    return CardInstance(card_id=CardId.DOUBT, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), effect_vars={'weak': 1}, instance_id=_get_next_id())

def make_enthralled() -> CardInstance:
    return CardInstance(card_id=CardId.ENTHRALLED, cost=2, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'eternal'}), instance_id=_get_next_id())

def make_folly() -> CardInstance:
    return CardInstance(card_id=CardId.FOLLY, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable', 'eternal', 'innate'}), instance_id=_get_next_id())

def make_greed() -> CardInstance:
    return CardInstance(card_id=CardId.GREED, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'eternal', 'unplayable'}), instance_id=_get_next_id())

def make_guilty() -> CardInstance:
    return CardInstance(card_id=CardId.GUILTY, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), effect_vars={'combats': 5}, instance_id=_get_next_id())

def make_injury() -> CardInstance:
    return CardInstance(card_id=CardId.INJURY, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), instance_id=_get_next_id())

def make_normality() -> CardInstance:
    return CardInstance(card_id=CardId.NORMALITY, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), effect_vars={'calc_base': 3, 'calc_extra': -1}, instance_id=_get_next_id())

def make_pain() -> CardInstance:
    return CardInstance(card_id=CardId.PAIN, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), instance_id=_get_next_id())

def make_parasite() -> CardInstance:
    return CardInstance(card_id=CardId.PARASITE, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), instance_id=_get_next_id())

def make_poor_sleep() -> CardInstance:
    return CardInstance(card_id=CardId.POOR_SLEEP, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable', 'retain'}), instance_id=_get_next_id())

def make_regret() -> CardInstance:
    return CardInstance(card_id=CardId.REGRET, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), instance_id=_get_next_id())

def make_shame() -> CardInstance:
    return CardInstance(card_id=CardId.SHAME, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), effect_vars={'frail': 1}, instance_id=_get_next_id())

def make_spore_mind() -> CardInstance:
    return CardInstance(card_id=CardId.SPORE_MIND, cost=1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())

def make_writhe() -> CardInstance:
    return CardInstance(card_id=CardId.WRITHE, cost=-1, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'innate', 'unplayable'}), instance_id=_get_next_id())

def make_deprecated_card() -> CardInstance:
    return CardInstance(card_id=CardId.DEPRECATED_CARD, cost=0, card_type=CardType.CURSE, target_type=TargetType.NONE, rarity=CardRarity.CURSE, keywords=frozenset({'unplayable'}), instance_id=_get_next_id())
...
def make_rip_and_tear(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.RIP_AND_TEAR, cost=1, card_type=CardType.ATTACK, target_type=TargetType.RANDOM_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=7, upgraded=upgraded, instance_id=_get_next_id())
...
def make_apotheosis(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.APOTHEOSIS, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.ANCIENT, upgraded=upgraded, keywords=frozenset({'exhaust', 'innate'}), instance_id=_get_next_id())
...
def make_apparition(upgraded: bool=False) -> CardInstance:
    ...
    return CardInstance(card_id=CardId.APPARITION, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.ANCIENT, upgraded=upgraded, keywords=kw, effect_vars={'intangible_power': 1}, instance_id=_get_next_id())
...
def make_brightest_flame(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BRIGHTEST_FLAME, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.ANCIENT, upgraded=upgraded, effect_vars={'cards': 2, 'energy': 2, 'max_hp': 1}, instance_id=_get_next_id())
...
def make_maul(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MAUL, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.ANCIENT, base_damage=5, upgraded=upgraded, effect_vars={'increase': 1}, instance_id=_get_next_id())
...
def make_neows_fury(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.NEOWS_FURY, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.ANCIENT, base_damage=10, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'cards': 2}, instance_id=_get_next_id())
...
def make_relax(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.RELAX, cost=3, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.ANCIENT, base_block=15, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'cards': 2, 'energy': 2}, instance_id=_get_next_id())
...
def make_whistle(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.WHISTLE, cost=3, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.ANCIENT, base_damage=33, upgraded=upgraded, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())
...
def wish_effect(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not ordered:
        return
    combat.request_card_choice(prompt='Choose a card from draw pile', cards=ordered, source_pile='draw', resolver=combat.move_card_to_hand)

def make_wish(upgraded: bool=False) -> CardInstance:
    kw = frozenset({'exhaust', 'retain'}) if upgraded else frozenset({'exhaust'})
    return CardInstance(card_id=CardId.WISH, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.ANCIENT, upgraded=upgraded, keywords=kw, instance_id=_get_next_id())
...
def make_byrd_swoop(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BYRD_SWOOP, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.EVENT, base_damage=14, upgraded=upgraded, instance_id=_get_next_id())
...
def make_caltrops(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CALTROPS, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.EVENT, upgraded=upgraded, effect_vars={'thorns_power': 3}, instance_id=_get_next_id())
...
def make_clash(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CLASH, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.EVENT, base_damage=14, upgraded=upgraded, instance_id=_get_next_id())

@register_effect(CardId.DISTRACTION)
def distraction_effect(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    generated = create_character_cards(combat.character_id, combat.combat_card_generation_rng, 1, card_type=CardType.SKILL, generation_context='combat', is_multiplayer=combat.is_multiplayer)
    if not generated:
        return
...
def make_distraction(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DISTRACTION, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.EVENT, upgraded=upgraded, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())
...
def dual_wield_effect(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    def _resolver(selected: CardInstance | None) -> None:
        ...
        combat._add_generated_cards_to_hand(clones)
    combat.request_card_choice(prompt='Choose an Attack or Power to copy', cards=candidates, source_pile='hand', resolver=_resolver)

def make_dual_wield(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DUAL_WIELD, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.EVENT, upgraded=upgraded, effect_vars={'cards': 1}, instance_id=_get_next_id())
...
def enlightenment_effect(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    for held in state.hand:
        ...
        elif not held.has_energy_cost_x:
            held.set_temporary_cost_for_turn(min(held.cost, 1))

def make_enlightenment(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ENLIGHTENMENT, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.EVENT, upgraded=upgraded, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())
...
def make_entrench(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ENTRENCH, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.EVENT, upgraded=upgraded, instance_id=_get_next_id())
...
def make_exterminate(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.EXTERMINATE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.EVENT, base_damage=3, upgraded=upgraded, effect_vars={'repeat': 4}, instance_id=_get_next_id())
...
def make_feeding_frenzy(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FEEDING_FRENZY_CARD, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.EVENT, upgraded=upgraded, effect_vars={'strength': 5}, instance_id=_get_next_id())
...
def make_hello_world(upgraded: bool=False) -> CardInstance:
    ...
    return CardInstance(card_id=CardId.HELLO_WORLD_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.EVENT, upgraded=upgraded, keywords=kw, instance_id=_get_next_id())
...
def mad_science_effect(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if card.card_type == CardType.ATTACK:
        ...
        if rider == 1:
            ...
            combat.apply_power_to(target, PowerId.VULNERABLE, card.effect_vars.get('sapping_vulnerable', 2), applier=owner)
        elif rider == 3:
    ...
    if card.card_type == CardType.SKILL:
        ...
        if rider == 4:
            ...
            generated = create_character_cards(combat.character_id, combat.combat_card_generation_rng, 1, distinct=True, generation_context='combat', is_multiplayer=combat.is_multiplayer)
            if generated:
...
def metamorphosis_effect(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    generated = create_character_cards(combat.character_id, combat.combat_card_generation_rng, card.effect_vars.get('cards', 3), card_type=CardType.ATTACK, distinct=False, generation_context='combat', is_multiplayer=combat.is_multiplayer)
    for generated_card in generated:
        generated_card.set_free_this_combat()
        combat.add_generated_card_to_creature_draw_pile(_owner(card, combat), generated_card, random_position=True)

def make_metamorphosis(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.METAMORPHOSIS, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.EVENT, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'cards': 3}, instance_id=_get_next_id())
...
def make_outmaneuver(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.OUTMANEUVER, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.EVENT, upgraded=upgraded, effect_vars={'energy': 2}, instance_id=_get_next_id())
...
def make_peck(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PECK, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.EVENT, base_damage=2, upgraded=upgraded, effect_vars={'repeat': 3}, instance_id=_get_next_id())
...
def make_rebound(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.REBOUND, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.EVENT, base_damage=9, upgraded=upgraded, instance_id=_get_next_id())
...
def make_squash(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SQUASH, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.EVENT, base_damage=10, upgraded=upgraded, effect_vars={'vulnerable': 2}, instance_id=_get_next_id())
...
def make_stack(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.STACK, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.EVENT, base_block=3 if upgraded else 0, upgraded=upgraded, effect_vars={'calc_base': 0, 'calc_extra': 1}, instance_id=_get_next_id())
...
def make_toric_toughness(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TORIC_TOUGHNESS, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.EVENT, base_block=5, upgraded=upgraded, effect_vars={'turns': 2}, instance_id=_get_next_id())
...
def make_fuel(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FUEL, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.STATUS, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'cards': 1, 'energy': 1}, instance_id=_get_next_id())
...
def make_giant_rock(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.GIANT_ROCK, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.STATUS, base_damage=16, upgraded=upgraded, instance_id=_get_next_id())
...
def make_luminesce(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.LUMINESCE, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.STATUS, upgraded=upgraded, keywords=frozenset({'exhaust', 'retain'}), effect_vars={'energy': 2}, instance_id=_get_next_id())
...
def make_minion_dive_bomb(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MINION_DIVE_BOMB, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.STATUS, base_damage=13, upgraded=upgraded, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())
...
def make_minion_sacrifice(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MINION_SACRIFICE, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.STATUS, base_block=9, upgraded=upgraded, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())
...
def make_minion_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MINION_STRIKE, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.STATUS, base_damage=6, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'cards': 1}, instance_id=_get_next_id())
...
def make_shiv(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SHIV, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.STATUS, base_damage=4, upgraded=upgraded, keywords=frozenset({'exhaust'}), tags=frozenset({CardTag.SHIV}), effect_vars={'calc_base': 0, 'calc_extra': 1}, instance_id=_get_next_id())
...
def make_soul(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SOUL, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.STATUS, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'cards': 2}, instance_id=_get_next_id())
...
def make_sovereign_blade(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SOVEREIGN_BLADE, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.STATUS, upgraded=upgraded, base_damage=10, keywords=frozenset({'retain'}), effect_vars={'calc_base': 0, 'calc_extra': 1, 'repeat': 1}, instance_id=_get_next_id())
...
def make_sweeping_gaze(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SWEEPING_GAZE, cost=0, card_type=CardType.ATTACK, target_type=TargetType.RANDOM_ENEMY, rarity=CardRarity.STATUS, base_damage=15 if upgraded else 10, upgraded=upgraded, keywords=frozenset({'ethereal', 'exhaust'}), effect_vars={'osty_damage': 10}, instance_id=_get_next_id())

def make_disintegration() -> CardInstance:
    return CardInstance(card_id=CardId.DISINTEGRATION, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, effect_vars={'disintegration_power': 6}, instance_id=_get_next_id())

def make_mind_rot() -> CardInstance:
    return CardInstance(card_id=CardId.MIND_ROT, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, effect_vars={'mind_rot_power': 1}, instance_id=_get_next_id())

def make_sloth_status() -> CardInstance:
    return CardInstance(card_id=CardId.SLOTH_STATUS, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, effect_vars={'sloth_power': 3}, instance_id=_get_next_id())

def make_waste_away() -> CardInstance:
    return CardInstance(card_id=CardId.WASTE_AWAY, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, effect_vars={'waste_away_power': 1}, instance_id=_get_next_id())

def make_byrdonis_egg() -> CardInstance:
    return CardInstance(card_id=CardId.BYRDONIS_EGG, cost=-1, card_type=CardType.QUEST, target_type=TargetType.NONE, rarity=CardRarity.QUEST, keywords=frozenset({'unplayable'}), instance_id=_get_next_id())
...
def make_lantern_key() -> CardInstance:
    return CardInstance(card_id=CardId.LANTERN_KEY, cost=-1, card_type=CardType.QUEST, target_type=TargetType.SELF, rarity=CardRarity.QUEST, keywords=frozenset({'unplayable'}), instance_id=_get_next_id())

def make_spoils_map() -> CardInstance:
    return CardInstance(card_id=CardId.SPOILS_MAP, cost=-1, card_type=CardType.QUEST, target_type=TargetType.SELF, rarity=CardRarity.QUEST, keywords=frozenset({'unplayable'}), effect_vars={'gold': 600}, instance_id=_get_next_id())
...
def void_after_card_drawn(card: CardInstance, drawn_card: CardInstance, from_hand_draw: bool, combat: CombatState) -> None:
...
def disintegration_chosen(card: CardInstance, combat: CombatState) -> None:
    owner = _owner(card, combat)
    combat.apply_power_to(owner, PowerId.DISINTEGRATION, card.effect_vars['disintegration_power'], applier=owner, source=card)
...
def mind_rot_chosen(card: CardInstance, combat: CombatState) -> None:
    owner = _owner(card, combat)
    combat.apply_power_to(owner, PowerId.MIND_ROT, card.effect_vars['mind_rot_power'], applier=owner, source=card)
...
def sloth_status_chosen(card: CardInstance, combat: CombatState) -> None:
    owner = _owner(card, combat)
    combat.apply_power_to(owner, PowerId.SLOTH, card.effect_vars['sloth_power'], applier=owner, source=card)
...
def waste_away_chosen(card: CardInstance, combat: CombatState) -> None:
    owner = _owner(card, combat)
    combat.apply_power_to(owner, PowerId.WASTE_AWAY, card.effect_vars['waste_away_power'], applier=owner, source=card)
...
def enthralled_should_play(card: CardInstance, played_card: CardInstance, owner_state, combat: CombatState, owner: Creature, is_auto_play: bool) -> bool:
...
def normality_should_play(card: CardInstance, played_card: CardInstance, owner_state, combat: CombatState, owner: Creature, is_auto_play: bool) -> bool:
...
def spoils_map_generated_map(card: CardInstance, run_state, act_map, act_index: int):
    ...
    return generate_spoils_act_map(num_rooms=act.num_rooms, rng=Rng(run_state.rng.seed, 'spoils_map'), ascension_level=run_state.ascension_level, act_index=act_index)
