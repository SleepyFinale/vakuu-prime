...
from sts2_env.cards.registry import register_after_card_entered_combat_hook, register_after_card_played_hook, register_after_attack_hook, register_after_death_hook, register_effect, register_late_effect, register_playability_hook, register_self_mutating_damage
from sts2_env.core.enums import CardId, CardTag, CardType, TargetType, CardRarity, ValueProp, PowerId, PowerType, PowerStackType
...
def _is_current_debuff(power_id: PowerId, amount: int) -> bool:
    ...
    if getattr(cls, 'stack_type', None) == PowerStackType.COUNTER and getattr(cls, 'allow_negative', False) and (amount < 0):
        return True
...
def flatten_after_card_entered_combat(card: CardInstance, entered_card: CardInstance, owner: Creature, combat: CombatState) -> None:
    if entered_card is not card:
        return
    if any((getattr(dealer, 'is_osty', False) and getattr(dealer, 'pet_owner', None) is owner and props.is_powered_attack() for dealer, _, props in combat._damage_events_this_turn)):
...
def reave(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    _deal_damage_single(card, combat, target)
    for _ in range(max(0, card.effect_vars.get('cards', 1))):
        combat.add_generated_card_to_creature_draw_pile(_owner(card, combat), make_soul(upgraded=card.upgraded), bottom_position=True)
...
def sculpting_strike(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    combat.request_card_choice(prompt='Choose a hand card to make Ethereal', cards=candidates, source_pile='hand', resolver=_resolver)
...
def capture_spirit(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    assert target is not None
    owner = _owner(card, combat)
    combat.deal_damage(dealer=owner, target=target, amount=card.effect_vars.get('damage', 3), props=ValueProp.UNBLOCKABLE | ValueProp.UNPOWERED | ValueProp.MOVE)
    for _ in range(max(0, card.effect_vars.get('cards', 3))):
        combat.add_generated_card_to_creature_draw_pile(_owner(card, combat), make_soul(), random_position=True)
...
def cleanse(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not cards:
        return
    combat.request_card_choice(prompt='Choose a draw-pile card to exhaust', cards=cards, source_pile='draw', resolver=combat.exhaust_card)
...
def dredge(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if max_cards <= 0:
        return
    combat.request_multi_card_choice(prompt='Choose discard cards to move to hand', cards=list(combat.discard_pile), source_pile='discard', resolver=lambda selected_cards: [combat.move_card_to_hand(selected) for selected in selected_cards], min_count=max_cards)
...
def melancholy_after_death(card: CardInstance, creature: Creature, was_removal_prevented: bool, combat: CombatState) -> None:
...
def banshees_cry_after_card_entered_combat(card: CardInstance, entered_card: CardInstance, owner: Creature, combat: CombatState) -> None:
    if entered_card is not card or card.combat_vars.get('_is_clone'):
        return
    amount = sum((1 for entry in combat._card_play_finished_entries_combat if entry.was_ethereal and getattr(entry.card, 'owner', None) is owner)) * card.effect_vars.get('energy', 2)
    if amount > 0:
        card.set_combat_cost(max(0, card.cost - amount))

@register_after_card_played_hook(CardId.BANSHEES_CRY)
def banshees_cry_after_card_played(card: CardInstance, played_card: CardInstance, owner: Creature, combat: CombatState) -> None:
...
def end_of_days(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    for enemy in list(combat.hittable_enemies):
        combat.apply_power_to(enemy, PowerId.DOOM, doom)
    doomed = [enemy for enemy in list(combat.hittable_enemies) if enemy.get_power_amount(PowerId.DOOM) > 0 and enemy.current_hp <= enemy.get_power_amount(PowerId.DOOM)]
...
def misery(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    assert target is not None
    original_debuffs = [(power_id, power.amount) for power_id, power in target.powers.items() if _is_current_debuff(power_id, power.amount)]
    _deal_damage_single(card, combat, target)
    owner = _owner(card, combat)
    for enemy in list(combat.hittable_enemies):
        if enemy is target:
        ...
        for power_id, amount in original_debuffs:
            cls = get_power_class(power_id)
            combat.apply_power_to(enemy, power_id, amount, applier=owner, source=card, ignore_next_instance=bool(cls is not None and getattr(cls, 'is_temporary', False)))
...
def seance(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    combat.request_multi_card_choice(prompt='Choose card(s) to transform into Soul', cards=cards, source_pile='draw', resolver=_resolver, min_count=card.effect_vars.get('cards', 1))
...
def squeeze(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    assert target is not None
    owner = _owner(card, combat)
    count = sum((1 for candidate in combat._all_cards_for_creature(owner) if candidate is not card and (CardTag.OSTY_ATTACK in getattr(candidate, 'tags', ()) or 'osty_attack' in getattr(candidate, 'tags', ()))))
...
def transfigure(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    combat.request_card_choice(prompt='Choose a hand card to grant Replay', cards=list(combat.hand), source_pile='hand', resolver=_resolver)
...
def make_strike_necrobinder(upgraded: bool=False) -> CardInstance:
    ...
    return CardInstance(card_id=CardId.STRIKE_NECROBINDER, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.BASIC, base_damage=6, upgraded=upgraded, instance_id=_get_next_id())

def make_defend_necrobinder(upgraded: bool=False) -> CardInstance:
    blk = 8 if upgraded else 5
    return CardInstance(card_id=CardId.DEFEND_NECROBINDER, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.BASIC, base_block=5, upgraded=upgraded, instance_id=_get_next_id())

def make_bodyguard(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BODYGUARD, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.BASIC, upgraded=upgraded, effect_vars={'summon': 5}, instance_id=_get_next_id())

def make_unleash(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.UNLEASH, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.BASIC, base_damage=6 if not upgraded else 9, upgraded=upgraded, instance_id=_get_next_id(), tags=frozenset({'osty_attack'}), effect_vars={'calc_base': 6, 'extra_damage': 1})

def make_afterlife(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.AFTERLIFE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, keywords=frozenset({'exhaust'}), upgraded=upgraded, effect_vars={'summon': 6}, instance_id=_get_next_id())

def make_blight_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BLIGHT_STRIKE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=8, upgraded=upgraded, instance_id=_get_next_id())

def make_defile(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DEFILE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=13, upgraded=upgraded, keywords=frozenset({'ethereal'}), instance_id=_get_next_id())

def make_defy(upgraded: bool=False) -> CardInstance:
    blk = 7 if upgraded else 6
    weak = 2 if upgraded else 1
    return CardInstance(card_id=CardId.DEFY, cost=1, card_type=CardType.SKILL, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_block=6, upgraded=upgraded, keywords=frozenset({'ethereal'}), effect_vars={'weak': 1}, instance_id=_get_next_id())

def make_drain_power(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DRAIN_POWER, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=10, upgraded=upgraded, effect_vars={'cards': 2}, instance_id=_get_next_id())

def make_fear(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FEAR, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=7, upgraded=upgraded, keywords=frozenset({'ethereal'}), effect_vars={'vulnerable': 1}, instance_id=_get_next_id())

def make_flatten(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FLATTEN, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=16 if upgraded else 12, upgraded=upgraded, tags=frozenset({'osty_attack'}), effect_vars={'osty_damage': 12}, instance_id=_get_next_id())

def make_grave_warden(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.GRAVE_WARDEN, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=8, upgraded=upgraded, effect_vars={'cards': 1}, instance_id=_get_next_id())

def make_graveblast(upgraded: bool=False) -> CardInstance:
    kw = frozenset() if upgraded else frozenset({'exhaust'})
    return CardInstance(card_id=CardId.GRAVEBLAST, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=4, upgraded=upgraded, keywords=kw, instance_id=_get_next_id())

def make_invoke(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.INVOKE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, upgraded=upgraded, effect_vars={'energy': 2, 'summon': 2}, instance_id=_get_next_id())

def make_negative_pulse(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.NEGATIVE_PULSE, cost=1, card_type=CardType.SKILL, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.COMMON, base_block=5, upgraded=upgraded, effect_vars={'doom': 7}, instance_id=_get_next_id())

def make_poke(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.POKE, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=9 if upgraded else 6, upgraded=upgraded, effect_vars={'osty_damage': 6}, instance_id=_get_next_id())

def make_pull_aggro(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PULL_AGGRO, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=7, upgraded=upgraded, effect_vars={'summon': 4}, instance_id=_get_next_id())

def make_reap(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.REAP, cost=3, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=27, upgraded=upgraded, keywords=frozenset({'retain'}), instance_id=_get_next_id())

def make_reave(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.REAVE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=9, upgraded=upgraded, effect_vars={'cards': 1}, instance_id=_get_next_id())

def make_scourge(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SCOURGE, cost=1, card_type=CardType.SKILL, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, upgraded=upgraded, effect_vars={'cards': 1, 'doom': 13}, instance_id=_get_next_id())

def make_sculpting_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SCULPTING_STRIKE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=9, upgraded=upgraded, instance_id=_get_next_id())

def make_snap(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SNAP, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=10 if upgraded else 7, upgraded=upgraded, effect_vars={'osty_damage': 7}, instance_id=_get_next_id())

def make_sow(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SOW, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.COMMON, base_damage=8, upgraded=upgraded, keywords=frozenset({'retain'}), instance_id=_get_next_id())

def make_wisp(upgraded: bool=False) -> CardInstance:
    kw = frozenset({'exhaust', 'retain'}) if upgraded else frozenset({'exhaust'})
    return CardInstance(card_id=CardId.WISP, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, upgraded=upgraded, keywords=kw, effect_vars={'energy': 1}, instance_id=_get_next_id())

def make_seance(upgraded: bool=False) -> CardInstance:
    kw = frozenset({'ethereal'})
    return CardInstance(card_id=CardId.SEANCE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=kw, effect_vars={'cards': 1}, instance_id=_get_next_id())

def make_capture_spirit(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CAPTURE_SPIRIT, cost=1, card_type=CardType.SKILL, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, upgraded=upgraded, effect_vars={'cards': 3}, instance_id=_get_next_id(), base_damage=3)

def make_cleanse(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CLEANSE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, effect_vars={'summon': 3}, instance_id=_get_next_id())

def make_dredge(upgraded: bool=False) -> CardInstance:
    kw = frozenset({'exhaust', 'retain'}) if upgraded else frozenset({'exhaust'})
    return CardInstance(card_id=CardId.DREDGE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, keywords=kw, effect_vars={'cards': 3}, instance_id=_get_next_id())

def make_dirge(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DIRGE, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, has_energy_cost_x=True, effect_vars={'summon': 3}, instance_id=_get_next_id())

def make_end_of_days(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.END_OF_DAYS, cost=3, card_type=CardType.SKILL, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.RARE, upgraded=upgraded, effect_vars={'doom': 29}, instance_id=_get_next_id())

def make_undeath(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.UNDEATH, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, base_block=7, instance_id=_get_next_id())

def make_reanimate(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.REANIMATE, cost=3, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'summon': 20}, instance_id=_get_next_id())

def make_necro_mastery(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.NECRO_MASTERY_CARD, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, effect_vars={'summon': 5}, instance_id=_get_next_id())

def make_sacrifice(upgraded: bool=False) -> CardInstance:
    kw = frozenset({'retain'})
    return CardInstance(card_id=CardId.SACRIFICE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=kw, instance_id=_get_next_id())

def make_legion_of_bone(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.LEGION_OF_BONE, cost=2, card_type=CardType.SKILL, target_type=TargetType.ALL_ALLIES, rarity=CardRarity.UNCOMMON, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'summon': 6}, instance_id=_get_next_id())

def make_severance(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SEVERANCE, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, upgraded=upgraded, base_damage=13, instance_id=_get_next_id())
...
def make_spur(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SPUR, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, keywords=frozenset({'retain'}), effect_vars={'heal': 5, 'summon': 3}, instance_id=_get_next_id())

def make_right_hand_hand(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.RIGHT_HAND_HAND, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, upgraded=upgraded, base_damage=6 if upgraded else 4, tags=frozenset({'osty_attack'}), effect_vars={'energy': 2, 'osty_damage': 4}, instance_id=_get_next_id())
...
_REFERENCE_FACTORY_CARD_IDS = (CardId.BORROWED_TIME, CardId.BURY, CardId.CALCIFY_CARD, CardId.COUNTDOWN_CARD, CardId.DANSE_MACABRE, CardId.DEATHBRINGER, CardId.DEATHS_DOOR, CardId.DEBILITATE_CARD, CardId.DELAY, CardId.ENFEEBLING_TOUCH, CardId.FRIENDSHIP, CardId.HAUNT, CardId.LETHALITY_CARD, CardId.MELANCHOLY, CardId.PAGESTORM, CardId.PARSE, CardId.PUTREFY, CardId.SHROUD, CardId.SLEIGHT_OF_FLESH, CardId.VEILPIERCER, CardId.BANSHEES_CRY, CardId.CALL_OF_THE_VOID, CardId.DEMESNE, CardId.DEVOUR_LIFE_CARD, CardId.HANG, CardId.MISERY, CardId.NEUROSURGE, CardId.OBLIVION, CardId.REAPER_FORM, CardId.SENTRY_MODE, CardId.SHARED_FATE, CardId.SPIRIT_OF_ASH, CardId.FORBIDDEN_GRIMOIRE)
