...
from sts2_env.cards.factory import card_preview, create_cards_from_ids, create_character_cards, create_distinct_character_cards, eligible_character_cards, eligible_registered_cards
...
from sts2_env.core.enums import CardId, CardType, TargetType, CardRarity, CardTag, PowerStackType, PowerType, ValueProp, PowerId
...
def dark_shackles(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    strength_loss = card.effect_vars.get(DARK_SHACKLES_STRENGTH_LOSS_KEY, DARK_SHACKLES_STRENGTH_LOSS)
...
def discovery(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    candidates = create_distinct_character_cards(combat.character_id, combat.combat_card_generation_rng, 3, generation_context='combat', is_multiplayer=combat.is_multiplayer)
    if not candidates:
        return
    ...
    combat.request_card_choice(prompt='Choose one of three cards', cards=candidates, source_pile='generated', resolver=_resolver, allow_skip=True)
...
def fisticuffs(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if combat.is_over:
        return
    block = calculate_block(result.total_damage + result.overkill_damage, owner, ValueProp.MOVE, combat, card_source=card)
    _gain_resolved_block(owner, block, combat)
...
def huddle_up(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    for teammate in [creature for creature in combat.get_teammates_of(_owner(card, combat)) if creature is not None and creature.is_alive and getattr(creature, 'is_player', False)]:
...
def jack_of_all_trades(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    colorless_ids = eligible_registered_cards(card_pool=CardPoolId.COLORLESS, exclude_ids={CardId.JACK_OF_ALL_TRADES}, generation_context='combat', is_multiplayer=combat.is_multiplayer)
    generated = create_cards_from_ids(colorless_ids, combat.combat_card_generation_rng, card.effect_vars.get(JACK_OF_ALL_TRADES_CARDS_KEY, JACK_OF_ALL_TRADES_CARDS), distinct=True)
...
def mind_blast(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    assert target is not None
    owner = _owner(card, combat)
    total_damage = card.effect_vars.get(MIND_BLAST_CALC_BASE_KEY, MIND_BLAST_CALC_BASE) + card.effect_vars.get(MIND_BLAST_EXTRA_DAMAGE_KEY, MIND_BLAST_EXTRA_DAMAGE) * len(combat.draw_pile)
    damage = calculate_damage(total_damage, owner, target, ValueProp.MOVE, combat)
    apply_damage(target, damage, ValueProp.MOVE, combat, owner)
...
def omnislice(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    assert target is not None
    owner = _owner(card, combat)
    base_damage = card.base_damage if card.base_damage is not None else card.effect_vars.get(OMNISLICE_DAMAGE_KEY, OMNISLICE_DAMAGE)
    ...
    for teammate in [enemy for enemy in combat.get_teammates_of(target) if enemy.is_alive]:
        teammate_damage = calculate_damage(splash_damage, owner, teammate, ValueProp.UNPOWERED | ValueProp.MOVE, combat)
        apply_damage(teammate, teammate_damage, ValueProp.UNPOWERED | ValueProp.MOVE, combat, owner)
...
def purity(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not cards:
        return
    combat.request_multi_card_choice(prompt='Choose any number of hand cards to exhaust', cards=cards, source_pile='hand', resolver=lambda selected_cards: [combat.exhaust_card(selected) for selected in selected_cards], min_count=0, max_count=card.effect_vars.get('cards', 3), allow_skip=True)
...
def seeker_strike(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    combat.request_card_choice(prompt='Choose one of the revealed cards', cards=candidates, source_pile='draw', resolver=combat.move_card_to_hand)
...
def splash(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    for cfg in ALL_CHARACTERS:
        if cfg.character_id.lower() == own_character:
            continue
        candidate_ids.extend(eligible_character_cards(cfg.character_id, card_type=CardType.ATTACK, generation_context='combat', is_multiplayer=combat.is_multiplayer))
    if not candidate_ids:
        return
    generated = create_cards_from_ids(candidate_ids, combat.combat_card_generation_rng, SPLASH_ATTACK_CHOICES, distinct=True)
    for generated_card in generated:
        if card.upgraded:
    ...
    combat.request_card_choice(prompt=SPLASH_CHOICE_PROMPT, cards=generated, source_pile=SPLASH_SOURCE_PILE, resolver=_resolver, allow_skip=True, owner=owner)
...
def thinking_ahead(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not combat.hand:
        return
    combat.request_card_choice(prompt='Choose a hand card to put back on draw pile', cards=list(combat.hand), source_pile='hand', resolver=lambda selected: combat.insert_card_into_draw_pile(selected, random_position=False))
...
def beat_down(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    discard = combat._zones_for_creature(owner)['discard']
...
def hidden_gem(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    valid_draw_cards = [c for c in combat.draw_pile if not c.is_unplayable and c.card_type not in (CardType.CURSE, CardType.QUEST)]
    candidates = [c for c in valid_draw_cards if c.card_type in (CardType.ATTACK, CardType.SKILL, CardType.POWER)]
    if not candidates:
...
def jackpot(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    assert target is not None
    _deal_damage_single(card, combat, target)
    zero_cost_ids = [card_id for card_id in eligible_registered_cards(module_name=None, card_type=None, generation_context='combat', is_multiplayer=combat.is_multiplayer) if card_preview(card_id).cost == 0 and (not card_preview(card_id).has_energy_cost_x)]
    generated = create_cards_from_ids([card_id for card_id in zero_cost_ids if card_id in set(get_character(combat.character_id).card_pool)], combat.combat_card_generation_rng, card.effect_vars.get(JACKPOT_CARDS_KEY, JACKPOT_CARDS), distinct=False)
    for generated_card in generated:
        if card.upgraded:
            combat.upgrade_card(generated_card)
        combat.add_generated_card_to_creature_hand(_owner(card, combat), generated_card)
...
def mimic(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if target is None:
        return
    owner = _owner(card, combat)
    total_block = card.effect_vars.get(MIMIC_CALC_BASE_KEY, MIMIC_CALC_BASE) + card.effect_vars.get(MIMIC_CALC_EXTRA_KEY, MIMIC_CALC_EXTRA) * target.block
...
def rend(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    assert target is not None
    debuffs = sum((1 for power_id, power in target.powers.items() if (cls := get_power_class(power_id)) is not None and _power_type_for_amount(cls, power.amount) == PowerType.DEBUFF and (not getattr(power, 'is_temporary', False))))
...
def secret_technique(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not candidates:
        return
    combat.request_card_choice(prompt='Choose a Skill from draw pile', cards=candidates, source_pile='draw', resolver=combat.move_card_to_hand)
...
def secret_weapon(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not candidates:
        return
    combat.request_card_choice(prompt='Choose an Attack from draw pile', cards=candidates, source_pile='draw', resolver=combat.move_card_to_hand)
...
def make_discovery(upgraded: bool=False) -> CardInstance:
    ...
    return CardInstance(card_id=CardId.DISCOVERY, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, keywords=keywords, instance_id=_get_next_id())

def make_dramatic_entrance(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DRAMATIC_ENTRANCE, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.UNCOMMON, base_damage=11, upgraded=upgraded, keywords=frozenset({'exhaust', 'innate'}), instance_id=_get_next_id())

def make_finesse(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FINESSE, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=4, upgraded=upgraded, effect_vars={'cards': 1}, instance_id=_get_next_id())

def make_flash_of_steel(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FLASH_OF_STEEL, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=5, upgraded=upgraded, effect_vars={'cards': 1}, instance_id=_get_next_id())

def make_shockwave(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SHOCKWAVE, cost=2, card_type=CardType.SKILL, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.UNCOMMON, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'power': 3}, instance_id=_get_next_id())

def make_master_of_strategy(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MASTER_OF_STRATEGY, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'cards': 3}, instance_id=_get_next_id())

def make_omnislice(upgraded: bool=False) -> CardInstance:
    damage = OMNISLICE_UPGRADED_DAMAGE if upgraded else OMNISLICE_DAMAGE
    return CardInstance(card_id=CardId.OMNISLICE, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=8, upgraded=upgraded, effect_vars={OMNISLICE_DAMAGE_KEY: damage}, instance_id=_get_next_id())

def make_secret_technique(upgraded: bool=False) -> CardInstance:
    keywords = frozenset() if upgraded else frozenset({'exhaust'})
    return CardInstance(card_id=CardId.SECRET_TECHNIQUE, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=keywords, instance_id=_get_next_id())

def make_secret_weapon(upgraded: bool=False) -> CardInstance:
    keywords = frozenset() if upgraded else frozenset({'exhaust'})
    return CardInstance(card_id=CardId.SECRET_WEAPON, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=keywords, instance_id=_get_next_id())

def make_thinking_ahead(upgraded: bool=False) -> CardInstance:
    keywords = frozenset() if upgraded else frozenset({'exhaust'})
    return CardInstance(card_id=CardId.THINKING_AHEAD, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, keywords=keywords, effect_vars={'cards': 2}, instance_id=_get_next_id())

def make_alchemize(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ALCHEMIZE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())

def make_hand_of_greed(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HAND_OF_GREED, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=20, upgraded=upgraded, effect_vars={'gold': 20}, instance_id=_get_next_id())

def make_hidden_gem(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HIDDEN_GEM, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, effect_vars={'replay': 2}, instance_id=_get_next_id())

def make_fisticuffs(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FISTICUFFS, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, upgraded=upgraded, base_damage=7, instance_id=_get_next_id())

def make_panic_button(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PANIC_BUTTON, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=30, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'turns': 2}, instance_id=_get_next_id())

def make_purity(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PURITY, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, keywords=frozenset({'retain', 'exhaust'}), effect_vars={'cards': 3}, instance_id=_get_next_id())

def make_ultimate_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ULTIMATE_STRIKE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=14, upgraded=upgraded, instance_id=_get_next_id())

def make_ultimate_defend(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ULTIMATE_DEFEND, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=11, upgraded=upgraded, instance_id=_get_next_id())

def make_believe_in_you(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BELIEVE_IN_YOU, cost=0, card_type=CardType.SKILL, target_type=TargetType.ANY_ALLY, rarity=CardRarity.UNCOMMON, effect_vars={'energy': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_catastrophe(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CATASTROPHE, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'cards': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_dark_shackles(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DARK_SHACKLES, cost=0, card_type=CardType.SKILL, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, keywords=frozenset({'exhaust'}), effect_vars={'strength_loss': 9}, upgraded=upgraded, instance_id=_get_next_id())

def make_equilibrium(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.EQUILIBRIUM, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=13, effect_vars={'equilibrium': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_fasten(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FASTEN, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'extra_block': 5}, upgraded=upgraded, instance_id=_get_next_id())

def make_intercept_card(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.INTERCEPT_CARD, cost=1, card_type=CardType.SKILL, target_type=TargetType.ANY_ALLY, rarity=CardRarity.UNCOMMON, base_block=9, upgraded=upgraded, instance_id=_get_next_id())

def make_jack_of_all_trades(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.JACK_OF_ALL_TRADES, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, keywords=frozenset({'exhaust'}), effect_vars={'cards': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_lift(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.LIFT, cost=1, card_type=CardType.SKILL, target_type=TargetType.ANY_ALLY, rarity=CardRarity.UNCOMMON, base_block=11, upgraded=upgraded, instance_id=_get_next_id())

def make_production(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PRODUCTION, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, keywords=frozenset() if upgraded else frozenset({'exhaust'}), effect_vars={'energy': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_prolong(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PROLONG, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, keywords=frozenset() if upgraded else frozenset({'exhaust'}), upgraded=upgraded, instance_id=_get_next_id())

def make_prowess(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PROWESS, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'dexterity': 1, 'strength': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_restlessness(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.RESTLESSNESS, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, keywords=frozenset({'retain'}), effect_vars={'cards': 2, 'energy': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_seeker_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SEEKER_STRIKE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=9, effect_vars={'cards': 3}, upgraded=upgraded, instance_id=_get_next_id())

def make_stratagem(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.STRATAGEM, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, instance_id=_get_next_id())

def make_tag_team(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TAG_TEAM, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=11, upgraded=upgraded, instance_id=_get_next_id())

def make_the_bomb_card(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.THE_BOMB_CARD, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'bomb_damage': 40, 'turns': 3}, upgraded=upgraded, instance_id=_get_next_id())

def make_thrumming_hatchet(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.THRUMMING_HATCHET, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=11, upgraded=upgraded, instance_id=_get_next_id())

def make_bolas(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BOLAS, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=3, upgraded=upgraded, instance_id=_get_next_id())

def make_calamity_card(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CALAMITY_CARD, cost=3, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, instance_id=_get_next_id())

def make_entropy(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ENTROPY, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=frozenset({'innate'}) if upgraded else frozenset(), effect_vars={'cards': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_eternal_armor(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ETERNAL_ARMOR, cost=3, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'plating': 9}, upgraded=upgraded, instance_id=_get_next_id())

def make_jackpot(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.JACKPOT, cost=3, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=25, effect_vars={'cards': 3}, upgraded=upgraded, instance_id=_get_next_id())

def make_mayhem_card(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MAYHEM_CARD, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, instance_id=_get_next_id())

def make_nostalgia_card(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.NOSTALGIA_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, instance_id=_get_next_id())
...
_REFERENCE_FACTORY_CARD_IDS = (CardId.AUTOMATION, CardId.BELIEVE_IN_YOU, CardId.CATASTROPHE, CardId.DARK_SHACKLES, CardId.EQUILIBRIUM, CardId.FASTEN, CardId.HUDDLE_UP, CardId.IMPATIENCE, CardId.INTERCEPT_CARD, CardId.JACK_OF_ALL_TRADES, CardId.LIFT, CardId.PANACHE_CARD, CardId.PREP_TIME, CardId.PRODUCTION, CardId.PROLONG, CardId.PROWESS, CardId.RESTLESSNESS, CardId.SEEKER_STRIKE, CardId.SPLASH, CardId.STRATAGEM, CardId.TAG_TEAM, CardId.THE_BOMB_CARD, CardId.THRUMMING_HATCHET, CardId.ANOINTED, CardId.BEACON_OF_HOPE, CardId.BEAT_DOWN, CardId.BOLAS, CardId.CALAMITY_CARD, CardId.ENTROPY, CardId.ETERNAL_ARMOR, CardId.JACKPOT, CardId.KNOCKDOWN, CardId.MAYHEM_CARD, CardId.NOSTALGIA_CARD, CardId.ROLLING_BOULDER, CardId.SCRAWL)
...
def make_coordinate_card(upgraded: bool=False) -> CardInstance:
    ...
    card.effect_vars[COORDINATE_STRENGTH_KEY] = COORDINATE_UPGRADED_STRENGTH if upgraded else COORDINATE_STRENGTH
    return card
...
def make_rend(upgraded: bool=False) -> CardInstance:
    ...
    card.effect_vars[REND_EXTRA_DAMAGE_KEY] = REND_UPGRADED_EXTRA_DAMAGE if upgraded else REND_EXTRA_DAMAGE
    return card
