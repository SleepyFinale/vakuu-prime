...
from sts2_env.cards.registry import register_after_card_drawn_hook, register_after_turn_end_hook, register_before_hand_draw_hook, register_effect, register_late_effect, register_self_mutating_damage
from sts2_env.core.card_pools import CardPoolId
from sts2_env.core.enums import CardId, CardType, TargetType, CardRarity, ValueProp, PowerId, CombatSide
...
def begone(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    combat.request_card_choice(prompt='Choose a hand card to transform', cards=list(combat.hand), source_pile='hand', resolver=_resolver)
...
def collision_course(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    combat._add_generated_cards_to_hand([make_debris()], owner=_owner(card, combat))
...
def cosmic_indifference(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not combat.discard_pile:
        return
    combat.request_card_choice(prompt='Choose a discard card to put on draw pile', cards=list(combat.discard_pile), source_pile='discard', resolver=lambda selected: combat.insert_card_into_draw_pile(selected, random_position=False))
...
def crescent_spear(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    assert target is not None
    owner = _owner(card, combat)
    count = sum((1 for candidate in combat._all_cards_for_creature(owner) if candidate.star_cost > 0 or candidate.has_star_cost_x))
...
def photon_cut(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not candidates:
        return
    combat.request_card_choice(prompt='Choose a hand card to put back on top of draw pile', cards=candidates, source_pile='hand', resolver=lambda selected: combat.insert_card_into_draw_pile(selected, random_position=False))
...
def charge(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    def _resolver(selected_cards: list[CardInstance]) -> None:
        for selected in selected_cards:
            ...
            combat.transform_card(selected, transformed)
    combat.request_multi_card_choice(prompt='Choose card(s) to transform into Minion Strike', cards=cards, source_pile='draw', resolver=_resolver, min_count=card.effect_vars.get('cards', 2))
...
def kingly_kick_after_card_drawn(card: CardInstance, drawn_card: CardInstance, from_hand_draw: bool, combat: CombatState) -> None:
...
def kingly_punch_after_card_drawn(card: CardInstance, drawn_card: CardInstance, from_hand_draw: bool, combat: CombatState) -> None:
...
def largesse(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    if target is None:
        return
    colorless_ids = eligible_registered_cards(card_pool=CardPoolId.COLORLESS, generation_context='combat', is_multiplayer=combat.is_multiplayer)
...
def quasar(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not generated:
        return
    combat.request_card_choice(prompt='Choose one colorless card', cards=generated, source_pile='generated', resolver=combat.move_card_to_hand, allow_skip=True)
...
def summon_forth(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    for pile in (combat.draw_pile, combat.discard_pile, combat.exhaust_pile):
        for blade in [c for c in list(pile) if c.card_id == CardId.SOVEREIGN_BLADE and getattr(c, 'owner', None) in (None, owner)]:
...
def bundle_of_joy(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    colorless_ids = eligible_registered_cards(card_pool=CardPoolId.COLORLESS, generation_context='combat', is_multiplayer=combat.is_multiplayer)
    generated = create_cards_from_ids(colorless_ids, combat.combat_card_generation_rng, card.effect_vars.get('cards', 3), distinct=True)
    for generated_card in generated:
...
def decisions_decisions(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    combat.request_card_choice(prompt='Choose a Skill to auto-play repeatedly', cards=candidates, source_pile='hand', resolver=_resolver)
...
def guards(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    def _resolver(selected_cards: list[CardInstance]) -> None:
        for selected in selected_cards:
            ...
            combat.transform_card(selected, transformed)
    combat.request_multi_card_choice(prompt='Choose any number of hand cards to transform', cards=list(combat.hand), source_pile='hand', resolver=_resolver, min_count=0, max_count=len(combat.hand), allow_skip=True)
...
def heirloom_hammer(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    combat.request_card_choice(prompt='Choose a colorless card to copy', cards=candidates, source_pile='hand', resolver=_resolver)
...
def make_strike_regent(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.STRIKE_REGENT, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.BASIC, base_damage=6, upgraded=upgraded, instance_id=_get_next_id())

def make_defend_regent(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DEFEND_REGENT, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.BASIC, base_block=5, upgraded=upgraded, instance_id=_get_next_id())

def make_falling_star(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FALLING_STAR, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.BASIC, base_damage=8, upgraded=upgraded, effect_vars={'weak': 1, 'vulnerable': 1}, star_cost=2, instance_id=_get_next_id())

def make_venerate(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.VENERATE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.BASIC, upgraded=upgraded, effect_vars={'stars': 2}, instance_id=_get_next_id())

def make_astral_pulse(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ASTRAL_PULSE, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.COMMON, base_damage=14, upgraded=upgraded, star_cost=3, instance_id=_get_next_id())

def make_beat_into_shape(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BEAT_INTO_SHAPE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, upgraded=upgraded, base_damage=5, effect_vars={'calc_base': 5, 'calc_extra': 5}, instance_id=_get_next_id())

def make_begone(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BEGONE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=5 if upgraded else 4, upgraded=upgraded, instance_id=_get_next_id())

def make_celestial_might(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CELESTIAL_MIGHT, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=6, upgraded=upgraded, effect_vars={'repeat': 3}, instance_id=_get_next_id())

def make_cloak_of_stars(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CLOAK_OF_STARS, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=7, star_cost=1, upgraded=upgraded, instance_id=_get_next_id())

def make_collision_course(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.COLLISION_COURSE, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=11, upgraded=upgraded, instance_id=_get_next_id())

def make_cosmic_indifference(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.COSMIC_INDIFFERENCE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=6, upgraded=upgraded, instance_id=_get_next_id())
...
def make_know_thy_place(upgraded: bool=False) -> CardInstance:
    ...
    return CardInstance(card_id=CardId.KNOW_THY_PLACE, cost=0, card_type=CardType.SKILL, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, upgraded=upgraded, keywords=kw, effect_vars={'weak': 1, 'vulnerable': 1}, instance_id=_get_next_id())

def make_patter(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PATTER, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=9, upgraded=upgraded, effect_vars={'vigor': 2}, instance_id=_get_next_id())

def make_photon_cut(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PHOTON_CUT, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=10, upgraded=upgraded, effect_vars={'cards': 1, 'put_back': 1}, instance_id=_get_next_id())

def make_refine_blade(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.REFINE_BLADE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, upgraded=upgraded, effect_vars={'energy': 1, 'forge': 9}, instance_id=_get_next_id())

def make_solar_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SOLAR_STRIKE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=9, upgraded=upgraded, effect_vars={'stars': 1}, instance_id=_get_next_id())

def make_spoils_of_battle(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SPOILS_OF_BATTLE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, upgraded=upgraded, effect_vars={'cards': 2, 'forge': 5}, instance_id=_get_next_id())

def make_wrought_in_war(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.WROUGHT_IN_WAR, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=7, upgraded=upgraded, effect_vars={'forge': 7}, instance_id=_get_next_id())
...
def make_big_bang(upgraded: bool=False) -> CardInstance:
    ...
    return CardInstance(card_id=CardId.BIG_BANG, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=kw, effect_vars={'cards': 1, 'energy': 1, 'stars': 1, 'forge': 5}, instance_id=_get_next_id())

def make_bundle_of_joy(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BUNDLE_OF_JOY, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=frozenset({'exhaust'}), effect_vars={'cards': 3}, instance_id=_get_next_id())

def make_crash_landing(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CRASH_LANDING, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.RARE, base_damage=21, upgraded=upgraded, instance_id=_get_next_id())

def make_bulwark(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BULWARK, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=13, upgraded=upgraded, effect_vars={'forge': 10}, instance_id=_get_next_id())

def make_charge(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CHARGE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, effect_vars={'cards': 2}, instance_id=_get_next_id())

def make_conqueror(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CONQUEROR, cost=1, card_type=CardType.SKILL, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, upgraded=upgraded, effect_vars={'forge': 3}, instance_id=_get_next_id())

def make_manifest_authority(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MANIFEST_AUTHORITY, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=7, upgraded=upgraded, instance_id=_get_next_id())

def make_guards(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.GUARDS, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())

def make_seeking_edge(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SEEKING_EDGE, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, effect_vars={'forge': 7}, instance_id=_get_next_id())

def make_the_smith(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.THE_SMITH, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, star_cost=4, effect_vars={'forge': 30}, instance_id=_get_next_id())

def make_convergence(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CONVERGENCE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, effect_vars={'energy': 1, 'stars': 1}, instance_id=_get_next_id())

def make_hidden_cache(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HIDDEN_CACHE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, upgraded=upgraded, effect_vars={'star_next_turn_power': 3, 'stars': 1}, instance_id=_get_next_id())

def make_resonance(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.RESONANCE, cost=1, card_type=CardType.SKILL, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.UNCOMMON, upgraded=upgraded, star_cost=3, effect_vars={'strength': 1}, instance_id=_get_next_id())

def make_summon_forth(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SUMMON_FORTH, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, effect_vars={'forge': 8}, instance_id=_get_next_id())

def make_decisions_decisions(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DECISIONS_DECISIONS, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, keywords=frozenset({'exhaust'}), star_cost=6, effect_vars={'cards': 3, 'repeat': 3}, instance_id=_get_next_id())

def make_quasar(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.QUASAR, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, star_cost=2, instance_id=_get_next_id())

def make_make_it_so(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MAKE_IT_SO, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, upgraded=upgraded, base_damage=6, effect_vars={'cards': 3}, instance_id=_get_next_id())

def make_heirloom_hammer(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HEIRLOOM_HAMMER, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, upgraded=upgraded, base_damage=20, effect_vars={'repeat': 1}, instance_id=_get_next_id())

def make_void_form(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.VOID_FORM, cost=3, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, effect_vars={'void_form_power': 2}, instance_id=_get_next_id())
...
_REFERENCE_FACTORY_CARD_IDS = (CardId.CRUSH_UNDER, CardId.ALIGNMENT, CardId.BLACK_HOLE, CardId.CHILD_OF_THE_STARS, CardId.DEVASTATE, CardId.FURNACE, CardId.GAMMA_BLAST, CardId.GLIMMER, CardId.HEGEMONY, CardId.KINGLY_KICK, CardId.KINGLY_PUNCH, CardId.KNOCKOUT_BLOW, CardId.MONOLOGUE_CARD, CardId.ORBIT, CardId.PARRY_CARD, CardId.PARTICLE_WALL, CardId.PILLAR_OF_CREATION, CardId.PROPHESIZE, CardId.REFLECT_CARD, CardId.ROYAL_GAMBLE, CardId.SHINING_STRIKE, CardId.SPECTRUM_SHIFT, CardId.TERRAFORMING, CardId.ARSENAL, CardId.BOMBARDMENT, CardId.COMET, CardId.FOREGONE_CONCLUSION, CardId.GENESIS, CardId.HAMMER_TIME, CardId.MONARCHS_GAZE_CARD, CardId.NEUTRON_AEGIS, CardId.ROYALTIES_CARD, CardId.SEVEN_STARS, CardId.SWORD_SAGE, CardId.TYRANNY_CARD, CardId.METEOR_SHOWER, CardId.THE_SEALED_THRONE)
