...
from sts2_env.cards.base import CardInstance, _get_next_id, increase_base_block, increase_base_damage, new_card_instance_id
from sts2_env.cards.factory import create_character_cards
from sts2_env.cards.registry import register_after_card_generated_for_combat_hook, register_effect, register_self_mutating_block, register_self_mutating_damage
from sts2_env.core.enums import CardId, CardType, TargetType, CardRarity, ValueProp, PowerId, OrbType
...
def _owner(card: CardInstance, combat: CombatState) -> Creature:
    return getattr(card, 'owner', None) or getattr(getattr(combat, 'active_card_source', None), 'owner', None) or combat.primary_player
...
def _channel_orb(combat: CombatState, orb_type: OrbType) -> None:
    ...
    owner = getattr(getattr(combat, 'active_card_source', None), 'owner', None) or combat.player
...
def _non_exhausted_status_cards(combat: CombatState, owner: Creature) -> list[CardInstance]:
    return [card for card in combat._all_cards_for_creature(owner, include_exhausted=False) if card.card_type == CardType.STATUS]
...
def _make_burn() -> CardInstance:
    return CardInstance(card_id=CardId.BURN, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'unplayable'}), instance_id=_get_next_id(), base_damage=2)

def _make_void() -> CardInstance:
    return CardInstance(card_id=CardId.VOID, cost=-1, card_type=CardType.STATUS, target_type=TargetType.NONE, rarity=CardRarity.STATUS, keywords=frozenset({'unplayable', 'ethereal'}), instance_id=_get_next_id(), effect_vars={'energy': 1})
...
def charge_battery(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    combat.apply_power_to(_owner(card, combat), PowerId.ENERGY_NEXT_TURN, card.effect_vars.get(CHARGE_BATTERY_ENERGY_KEY, CHARGE_BATTERY_ENERGY_NEXT_TURN))
...
def focused_strike(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    dmg = calculate_damage(card.base_damage, _owner(card, combat), target, ValueProp.MOVE, combat)
    apply_damage(target, dmg, ValueProp.MOVE, combat, _owner(card, combat))
    combat.apply_power_to(_owner(card, combat), PowerId.FOCUSED_STRIKE, card.effect_vars.get(FOCUSED_STRIKE_POWER_KEY, FOCUSED_STRIKE_POWER))
...
def hologram(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not combat.discard_pile:
        return
    combat.request_card_choice(prompt='Choose a discard card to return to hand', cards=list(combat.discard_pile), source_pile='discard', resolver=combat.move_card_to_hand)
...
def lightning_rod(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    blk = calculate_block(card.base_block, _owner(card, combat), ValueProp.MOVE, combat, card_source=card)
    _gain_resolved_block(_owner(card, combat), blk, combat)
    combat.apply_power_to(_owner(card, combat), PowerId.LIGHTNING_ROD, card.effect_vars.get(LIGHTNING_ROD_POWER_KEY, LIGHTNING_ROD_POWER))
...
def bulk_up(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    owner = _owner(card, combat)
    combat.apply_power_to(owner, PowerId.STRENGTH, card.effect_vars.get(BULK_UP_STRENGTH_KEY, BULK_UP_POWER))
    combat.apply_power_to(owner, PowerId.DEXTERITY, card.effect_vars.get(BULK_UP_DEXTERITY_KEY, BULK_UP_POWER))
...
def feral(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    combat.apply_power_to(_owner(card, combat), PowerId.FERAL, card.effect_vars.get(FERAL_POWER_KEY, FERAL_POWER))
...
def ftl(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if previous_plays < card.effect_vars.get(FTL_PLAY_MAX_KEY, FTL_UPGRADED_PLAY_MAX if card.upgraded else FTL_PLAY_MAX):
...
def hailstorm(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    combat.apply_power_to(_owner(card, combat), PowerId.HAILSTORM, card.effect_vars.get(HAILSTORM_POWER_KEY, HAILSTORM_POWER))
...
def rocket_punch_after_card_generated_for_combat(card: CardInstance, generated_card: CardInstance, added_by_player: bool, combat: CombatState) -> None:
...
def scavenge(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    def _resolver(selected: CardInstance | None) -> None:
        ...
        combat.apply_power_to(owner, PowerId.ENERGY_NEXT_TURN, energy)
    combat.request_card_choice(prompt='Choose a hand card to exhaust', cards=candidates, source_pile='hand', resolver=_resolver)
...
def scrape(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    combat._draw_cards(draw)
    to_discard = [c for c in combat.hand[-draw:] if c.cost != 0 or c.has_energy_cost_x or int(c.combat_vars.get('_turn_star_cost_override', c.combat_vars.get('_combat_star_cost_override', c.star_cost))) > 0 or c.has_star_cost_x]
    for c in to_discard:
...
def thunder_card(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    combat.apply_power_to(_owner(card, combat), PowerId.THUNDER, card.effect_vars.get(THUNDER_POWER_KEY, THUNDER_POWER))

@register_effect(CardId.WHITE_NOISE)
def white_noise(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    generated = create_character_cards(combat.character_id, combat.combat_card_generation_rng, 1, card_type=CardType.POWER, generation_context='combat', is_multiplayer=combat.is_multiplayer)
    if not generated:
        return
...
def coolant(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    combat.apply_power_to(_owner(card, combat), PowerId.COOLANT, card.effect_vars.get(COOLANT_POWER_KEY, COOLANT_POWER))

@register_effect(CardId.CREATIVE_AI_CARD)
def creative_ai(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    combat.apply_power_to(_owner(card, combat), PowerId.CREATIVE_AI, card.effect_vars.get(CREATIVE_AI_POWER_KEY, CREATIVE_AI_POWER))

@register_effect(CardId.DEFRAGMENT)
def defragment(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    combat.apply_power_to(_owner(card, combat), PowerId.FOCUS, card.effect_vars.get(DEFRAGMENT_FOCUS_KEY, DEFRAGMENT_FOCUS))
...
def hyperbeam(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    for enemy in combat.hittable_enemies:
        dmg = calculate_damage(card.base_damage, _owner(card, combat), enemy, ValueProp.MOVE, combat)
        apply_damage(enemy, dmg, ValueProp.MOVE, combat, _owner(card, combat))
    combat.apply_power_to(_owner(card, combat), PowerId.FOCUS, -card.effect_vars.get(HYPERBEAM_FOCUS_KEY, HYPERBEAM_FOCUS))
...
def machine_learning(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    combat.apply_power_to(_owner(card, combat), PowerId.MACHINE_LEARNING, card.effect_vars.get(MACHINE_LEARNING_CARDS_KEY, MACHINE_LEARNING_CARDS))
...
def voltaic(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    owner = _owner(card, combat)
    channel_count = sum((1 for channel_owner, orb_type in getattr(combat, '_orb_channel_events_combat', ()) if channel_owner is owner and orb_type == OrbType.LIGHTNING))
...
def biased_cognition(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    combat.apply_power_to(_owner(card, combat), PowerId.FOCUS, card.effect_vars.get(BIASED_COGNITION_FOCUS_KEY, BIASED_COGNITION_FOCUS))
    combat.apply_power_to(_owner(card, combat), PowerId.BIASED_COGNITION, card.effect_vars.get(BIASED_COGNITION_POWER_KEY, BIASED_COGNITION_POWER))
...
def make_strike_defect(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.STRIKE_DEFECT, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.BASIC, base_damage=6, upgraded=upgraded, instance_id=_get_next_id())

def make_defend_defect(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DEFEND_DEFECT, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.BASIC, base_block=5, upgraded=upgraded, instance_id=_get_next_id())

def make_zap(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ZAP, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.BASIC, upgraded=upgraded, instance_id=_get_next_id())

def make_dualcast(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DUALCAST, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.BASIC, upgraded=upgraded, instance_id=_get_next_id())

def make_ball_lightning(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BALL_LIGHTNING, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=7, upgraded=upgraded, instance_id=_get_next_id())

def make_barrage(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BARRAGE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=5, effect_vars={'calc_base': 0, 'calc_extra': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_beam_cell(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BEAM_CELL, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=3, effect_vars={'vulnerable': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_boost_away(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BOOST_AWAY, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=6, upgraded=upgraded, instance_id=_get_next_id())

def make_charge_battery(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CHARGE_BATTERY, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=7, effect_vars={'energy': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_claw(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CLAW, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=3, effect_vars={'increase': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_cold_snap(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.COLD_SNAP, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=6, upgraded=upgraded, instance_id=_get_next_id())

def make_compile_driver(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.COMPILE_DRIVER, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=7, effect_vars={'calc_base': 0, 'calc_extra': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_coolheaded() -> CardInstance:
    return CardInstance(card_id=CardId.COOLHEADED, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, effect_vars={'cards': 1}, instance_id=_get_next_id())

def make_focused_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FOCUSED_STRIKE_CARD, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=9, effect_vars={'focus_power': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_go_for_the_eyes(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.GO_FOR_THE_EYES, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=3, effect_vars={'weak': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_gunk_up(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.GUNK_UP, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=4, effect_vars={'repeat': 3}, upgraded=upgraded, instance_id=_get_next_id())

def make_hologram() -> CardInstance:
    return CardInstance(card_id=CardId.HOLOGRAM, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=3, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())

def make_hotfix(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HOTFIX, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, effect_vars={'focus_power': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_leap(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.LEAP, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=9, upgraded=upgraded, instance_id=_get_next_id())

def make_lightning_rod(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.LIGHTNING_ROD, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=4, effect_vars={'lightning_rod_power': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_momentum_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MOMENTUM_STRIKE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=10, upgraded=upgraded, instance_id=_get_next_id())

def make_sweeping_beam(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SWEEPING_BEAM, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.COMMON, base_damage=6, effect_vars={'cards': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_turbo(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TURBO, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, effect_vars={'energy': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_uproar(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.UPROAR, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=5, upgraded=upgraded, instance_id=_get_next_id())

def make_boot_sequence(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BOOT_SEQUENCE, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=10, keywords=frozenset({'innate', 'exhaust'}), upgraded=upgraded, instance_id=_get_next_id())

def make_bulk_up(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BULK_UP, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'dexterity': 2, 'orb_slots': 1, 'strength': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_capacitor(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CAPACITOR, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, effect_vars={'repeat': 2}, instance_id=_get_next_id())

def make_chaos(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CHAOS, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'repeat': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_chill(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CHILL, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, keywords=frozenset() if upgraded else frozenset({'exhaust'}), upgraded=upgraded, instance_id=_get_next_id())

def make_compact() -> CardInstance:
    return CardInstance(card_id=CardId.COMPACT, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=6, instance_id=_get_next_id())

def make_darkness(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DARKNESS_CARD, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, effect_vars={'passives': 2 if upgraded else 1}, instance_id=_get_next_id())

def make_double_energy(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DOUBLE_ENERGY, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, keywords=frozenset({'exhaust'}), upgraded=upgraded, instance_id=_get_next_id())

def make_energy_surge(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ENERGY_SURGE, cost=1, card_type=CardType.SKILL, target_type=TargetType.ALL_ALLIES, rarity=CardRarity.UNCOMMON, keywords=frozenset({'exhaust'}), effect_vars={'energy': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_fusion(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FUSION, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, instance_id=_get_next_id())

def make_glacier(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.GLACIER, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=6, upgraded=upgraded, instance_id=_get_next_id())

def make_glasswork(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.GLASSWORK, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=5, upgraded=upgraded, instance_id=_get_next_id())

def make_hailstorm(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HAILSTORM, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'hailstorm_power': 6}, upgraded=upgraded, instance_id=_get_next_id())

def make_iteration(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ITERATION_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'iteration_power': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_loop() -> CardInstance:
    return CardInstance(card_id=CardId.LOOP_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'loop': 1}, instance_id=_get_next_id())

def make_null(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.NULL_CARD, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=10, effect_vars={'weak': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_overclock(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.OVERCLOCK, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'cards': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_refract(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.REFRACT, cost=3, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=9, effect_vars={'repeat': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_rocket_punch(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ROCKET_PUNCH, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=13, effect_vars={'cards': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_scavenge() -> CardInstance:
    return CardInstance(card_id=CardId.SCAVENGE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'energy': 2}, instance_id=_get_next_id())

def make_scrape(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SCRAPE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=7, effect_vars={'cards': 4}, upgraded=upgraded, instance_id=_get_next_id())

def make_shadow_shield(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SHADOW_SHIELD, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=11, upgraded=upgraded, instance_id=_get_next_id())

def make_skim(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SKIM, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'cards': 3}, upgraded=upgraded, instance_id=_get_next_id())

def make_smokestack(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SMOKESTACK, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'smokestack_power': 5}, upgraded=upgraded, instance_id=_get_next_id())

def make_storm(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.STORM_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'storm_power': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_subroutine(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SUBROUTINE, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, upgraded=upgraded, instance_id=_get_next_id())

def make_sunder() -> CardInstance:
    return CardInstance(card_id=CardId.SUNDER, cost=3, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=24, effect_vars={'energy': 3}, instance_id=_get_next_id())

def make_synchronize(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SYNCHRONIZE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, keywords=frozenset() if upgraded else frozenset({'exhaust'}), effect_vars={'calc_base': 0, 'calc_extra': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_synthesis(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SYNTHESIS, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=12, upgraded=upgraded, instance_id=_get_next_id())

def make_tempest(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TEMPEST, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, has_energy_cost_x=True, upgraded=upgraded, instance_id=_get_next_id())

def make_tesla_coil(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TESLA_COIL, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=3, upgraded=upgraded, instance_id=_get_next_id())

def make_thunder(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.THUNDER_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'thunder_power': 6}, upgraded=upgraded, instance_id=_get_next_id())

def make_white_noise() -> CardInstance:
    return CardInstance(card_id=CardId.WHITE_NOISE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, keywords=frozenset({'exhaust'}), instance_id=_get_next_id())

def make_adaptive_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ADAPTIVE_STRIKE, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=18, upgraded=upgraded, instance_id=_get_next_id())

def make_all_for_one(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ALL_FOR_ONE, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=10, upgraded=upgraded, instance_id=_get_next_id())

def make_buffer(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BUFFER_CARD, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'buffer_power': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_consuming_shadow(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CONSUMING_SHADOW, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, effect_vars={'consuming_shadow_power': 1, 'repeat': 2}, instance_id=_get_next_id())

def make_coolant(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.COOLANT, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'coolant_power': 2}, upgraded=upgraded, instance_id=_get_next_id())

def make_creative_ai(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CREATIVE_AI_CARD, cost=3, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'creative_ai': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_defragment(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DEFRAGMENT, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'focus_power': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_echo_form(upgraded: bool=False) -> CardInstance:
    keywords = frozenset() if upgraded else frozenset({'ethereal'})
    return CardInstance(card_id=CardId.ECHO_FORM_CARD, cost=3, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=keywords, effect_vars={'echo_form': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_flak_cannon() -> CardInstance:
    return CardInstance(card_id=CardId.FLAK_CANNON, cost=2, card_type=CardType.ATTACK, target_type=TargetType.RANDOM_ENEMY, rarity=CardRarity.RARE, base_damage=8, effect_vars={'calc_base': 0, 'calc_extra': 1}, instance_id=_get_next_id())

def make_genetic_algorithm(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.GENETIC_ALGORITHM, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, base_block=GENETIC_ALGORITHM_BLOCK, keywords=frozenset({'exhaust'}), effect_vars={'increase': 3}, upgraded=upgraded, instance_id=_get_next_id())

def make_helix_drill(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HELIX_DRILL, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=3, effect_vars={'calc_base': 0, 'calc_extra': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_hyperbeam(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HYPERBEAM, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.RARE, base_damage=26, effect_vars={'focus_power': 3}, upgraded=upgraded, instance_id=_get_next_id())

def make_ice_lance(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ICE_LANCE, cost=3, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=19, effect_vars={'repeat': 3}, upgraded=upgraded, instance_id=_get_next_id())

def make_ignition(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.IGNITION, cost=1, card_type=CardType.SKILL, target_type=TargetType.ANY_ALLY, rarity=CardRarity.RARE, keywords=frozenset() if upgraded else frozenset({'exhaust'}), upgraded=upgraded, instance_id=_get_next_id())

def make_machine_learning(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MACHINE_LEARNING_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'cards': 1}, keywords=frozenset({'innate'}) if upgraded else frozenset(), upgraded=upgraded, instance_id=_get_next_id())

def make_meteor_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.METEOR_STRIKE, cost=5, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=24, upgraded=upgraded, instance_id=_get_next_id())

def make_modded(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MODDED, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'cards': 1, 'repeat': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_multi_cast(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MULTI_CAST, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, has_energy_cost_x=True, upgraded=upgraded, instance_id=_get_next_id())

def make_rainbow(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.RAINBOW, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=frozenset() if upgraded else frozenset({'exhaust'}), upgraded=upgraded, instance_id=_get_next_id())

def make_reboot(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.REBOOT, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=frozenset({'exhaust'}), effect_vars={'cards': 4}, upgraded=upgraded, instance_id=_get_next_id())

def make_shatter(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SHATTER, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.RARE, base_damage=11, upgraded=upgraded, instance_id=_get_next_id())

def make_signal_boost(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SIGNAL_BOOST, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=frozenset({'exhaust'}), effect_vars={'signal_boost_power': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_spinner(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SPINNER_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'spinner_power': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_supercritical(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SUPERCRITICAL, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=frozenset({'exhaust'}), effect_vars={'energy': 4}, upgraded=upgraded, instance_id=_get_next_id())

def make_trash_to_treasure(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TRASH_TO_TREASURE, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=frozenset({'innate'}) if upgraded else frozenset(), upgraded=upgraded, instance_id=_get_next_id())

def make_voltaic(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.VOLTAIC, cost=3, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=frozenset() if upgraded else frozenset({'exhaust'}), effect_vars={'calc_base': 0, 'calc_extra': 1}, upgraded=upgraded, instance_id=_get_next_id())

def make_biased_cognition(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BIASED_COGNITION_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.ANCIENT, effect_vars={'biased_cognition_power': 1, 'focus_power': 4}, upgraded=upgraded, instance_id=_get_next_id())

def make_quadcast(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.QUADCAST, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.ANCIENT, effect_vars={'repeat': 4}, upgraded=upgraded, instance_id=_get_next_id())
