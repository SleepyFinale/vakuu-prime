"""All 88 Ironclad card effects and factories.

Covers: 3 Basic, 21 Common, 32 Uncommon, 30 Rare, 2 Ancient cards.
...
from sts2_env.cards.registry import register_after_card_entered_combat_hook, register_before_card_played_hook, register_before_hand_draw_hook, register_effect, register_playability_hook, register_self_mutating_damage
from sts2_env.core.enums import CardId, CardTag, CardType, TargetType, CardRarity, ValueProp, PowerId, CombatSide
...
def _deal_damage_to_target(card: CardInstance, combat: CombatState, target: Creature) -> None:
...
def _deal_damage_all_enemies(card: CardInstance, combat: CombatState) -> None:
...
def _calculate_untargeted_card_damage(card: CardInstance, owner: Creature, combat: CombatState) -> int:
...
def _self_hp_loss(card: CardInstance, combat: CombatState, amount: int) -> None:
    ...
    owner = _owner(card, combat)
    apply_damage(owner, amount, ValueProp.UNBLOCKABLE | ValueProp.UNPOWERED | ValueProp.MOVE, combat, owner)
...
def make_strike_ironclad(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.STRIKE_IRONCLAD, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.BASIC, base_damage=6, upgraded=upgraded, instance_id=_get_next_id())
...
def make_defend_ironclad(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DEFEND_IRONCLAD, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.BASIC, base_block=5, upgraded=upgraded, instance_id=_get_next_id())
...
def make_bash(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BASH, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.BASIC, base_damage=8, effect_vars={'vulnerable': 2}, upgraded=upgraded, instance_id=_get_next_id())

@register_effect(CardId.ANGER)
def anger(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    assert target is not None
    _deal_damage_to_target(card, combat, target)
    copy = CardInstance(card_id=CardId.ANGER, cost=card.cost, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=card.base_damage, upgraded=card.upgraded, instance_id=_get_next_id())
...
def make_anger(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ANGER, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=6, upgraded=upgraded, instance_id=_get_next_id())
...
def armaments(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if card.upgraded:
        ...
        return
    combat.request_card_choice(prompt='Choose a hand card to upgrade', cards=candidates, source_pile='hand', resolver=combat.upgrade_card)

def make_armaments(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ARMAMENTS, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=5, upgraded=upgraded, instance_id=_get_next_id())
...
def make_blood_wall(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BLOOD_WALL, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=16, effect_vars={'hp_loss': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_bloodletting(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BLOODLETTING, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, effect_vars={'energy': 2, 'hp_loss': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_body_slam(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BODY_SLAM, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=0, effect_vars={'calc_base': 0, 'extra_damage': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_breakthrough(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BREAKTHROUGH, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.COMMON, base_damage=9, effect_vars={'hp_loss': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_havoc(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HAVOC, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, upgraded=upgraded, instance_id=_get_next_id())
...
def headbutt(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not combat.discard_pile:
        return
    combat.request_card_choice(prompt='Choose a discard card to put on top of draw pile', cards=list(combat.discard_pile), source_pile='discard', resolver=lambda selected: combat.insert_card_into_draw_pile(selected, random_position=False))

def make_headbutt(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HEADBUTT, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=9, upgraded=upgraded, instance_id=_get_next_id())
...
def make_iron_wave(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.IRON_WAVE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=5, base_block=5, upgraded=upgraded, instance_id=_get_next_id())
...
def make_molten_fist(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MOLTEN_FIST, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=10, keywords=frozenset({'exhaust'}), upgraded=upgraded, instance_id=_get_next_id())
...
def make_perfected_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PERFECTED_STRIKE, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=6, effect_vars={'calc_base': 6, 'extra_damage': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_pommel_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.POMMEL_STRIKE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=9, effect_vars={'cards': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_setup_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SETUP_STRIKE_CARD, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=7, effect_vars={'strength': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_shrug_it_off(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SHRUG_IT_OFF, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=8, effect_vars={'cards': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_sword_boomerang(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SWORD_BOOMERANG, cost=1, card_type=CardType.ATTACK, target_type=TargetType.RANDOM_ENEMY, rarity=CardRarity.COMMON, base_damage=3, effect_vars={'repeat': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_thunderclap(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.THUNDERCLAP, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.COMMON, base_damage=4, effect_vars={'vulnerable': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_tremble(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TREMBLE, cost=1, card_type=CardType.SKILL, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, effect_vars={'vulnerable': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def true_grit(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    if not combat.hand:
        return
    if card.upgraded:
        combat.request_card_choice(prompt='Choose a hand card to exhaust', cards=list(combat.hand), source_pile='hand', resolver=combat.exhaust_card)
        return
...
def make_true_grit(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TRUE_GRIT, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.COMMON, base_block=7, upgraded=upgraded, instance_id=_get_next_id())
...
def make_twin_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TWIN_STRIKE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.COMMON, base_damage=5, upgraded=upgraded, instance_id=_get_next_id())
...
def make_ashen_strike(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ASHEN_STRIKE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=6, effect_vars={'calc_base': 6, 'extra_damage': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_battle_trance(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BATTLE_TRANCE, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'cards': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_bludgeon(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BLUDGEON, cost=3, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=32, upgraded=upgraded, instance_id=_get_next_id())
...
def make_bully(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BULLY, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=4, effect_vars={'calc_base': 4, 'extra_damage': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def burning_pact(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    def _resolver(selected: CardInstance | None) -> None:
        ...
        _draw_cards(combat, card.effect_vars.get('cards', 2), owner)
    combat.request_card_choice(prompt='Choose a hand card to exhaust', cards=list(combat.hand), source_pile='hand', resolver=_resolver)

def make_burning_pact(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BURNING_PACT, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'cards': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_demonic_shield(upgraded: bool=False) -> CardInstance:
    ...
    return CardInstance(card_id=CardId.DEMONIC_SHIELD, cost=0, card_type=CardType.SKILL, target_type=TargetType.ANY_ALLY, rarity=CardRarity.UNCOMMON, effect_vars={'hp_loss': 1, 'calc_base': 0, 'calc_extra': 1}, keywords=kw, upgraded=upgraded, instance_id=_get_next_id())
...
def make_dismantle(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DISMANTLE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=8, effect_vars={'hits': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_dominate(upgraded: bool=False) -> CardInstance:
    kw = frozenset() if upgraded else frozenset({'exhaust'})
    return CardInstance(card_id=CardId.DOMINATE, cost=1, card_type=CardType.SKILL, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, effect_vars={'strength_per_vulnerable': 1, 'vulnerable': 1}, keywords=kw, upgraded=upgraded, instance_id=_get_next_id())
...
def make_drum_of_battle(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DRUM_OF_BATTLE_CARD, cost=0, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'cards': 2, 'drum_of_battle_power': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_evil_eye(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.EVIL_EYE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=8, upgraded=upgraded, instance_id=_get_next_id())

@register_effect(CardId.EXPECT_A_FIGHT)
def expect_a_fight(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    owner = _owner(card, combat)
    combat.apply_power_to(owner, PowerId.NO_ENERGY_GAIN, 1, applier=owner, source=card)

def make_expect_a_fight(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.EXPECT_A_FIGHT, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'energy': 0, 'calc_base': 0, 'calc_extra': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_feel_no_pain(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FEEL_NO_PAIN_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'power': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_fight_me(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FIGHT_ME, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=5, effect_vars={'enemy_strength': 1, 'repeat': 2, 'strength': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_flame_barrier(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FLAME_BARRIER_CARD, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, base_block=12, effect_vars={'damage_back': 4}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_forgotten_ritual(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FORGOTTEN_RITUAL, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'energy': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_grapple(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.GRAPPLE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=7, effect_vars={'grapple_power': 5}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_hemokinesis(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HEMOKINESIS, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=15, effect_vars={'hp_loss': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_howl_from_beyond(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HOWL_FROM_BEYOND, cost=3, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.UNCOMMON, base_damage=16, upgraded=upgraded, instance_id=_get_next_id())
...
def infernal_blade(card: CardInstance, combat: CombatState, target: Creature | None) -> None:
    ...
    generated = create_character_cards(combat.character_id, combat.combat_card_generation_rng, 1, card_type=CardType.ATTACK, is_multiplayer=combat.is_multiplayer)
    if not generated:
        return
...
def make_infernal_blade(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.INFERNAL_BLADE, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, keywords=frozenset({'exhaust'}), upgraded=upgraded, instance_id=_get_next_id())
...
def make_inferno(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.INFERNO_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'inferno_power': 6}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_inflame(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.INFLAME, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'strength': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_juggling(upgraded: bool=False) -> CardInstance:
    ...
    return CardInstance(card_id=CardId.JUGGLING_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, keywords=kw, upgraded=upgraded, instance_id=_get_next_id())
...
def make_pillage(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PILLAGE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=6, upgraded=upgraded, instance_id=_get_next_id())
...
def make_rage(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.RAGE_CARD, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'power': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_rampage(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.RAMPAGE, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=9, effect_vars={'increase': 5}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_rupture(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.RUPTURE_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'strength': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_second_wind(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.SECOND_WIND, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'block': 7 if upgraded else 5}, upgraded=upgraded, instance_id=_get_next_id(), base_block=5)
...
def make_stampede(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.STAMPEDE_CARD, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'power': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def stomp_after_card_entered_combat(card: CardInstance, entered_card: CardInstance, owner: Creature, combat: CombatState) -> None:
...
def stomp_before_card_played(card: CardInstance, played_card: CardInstance, owner: Creature, combat: CombatState) -> None:
...
def make_stomp(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.STOMP, cost=3, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.UNCOMMON, base_damage=12, upgraded=upgraded, instance_id=_get_next_id())
...
def make_stone_armor(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.STONE_ARMOR, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'plating': 4}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_taunt(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TAUNT, cost=1, card_type=CardType.SKILL, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_block=7, effect_vars={'vulnerable': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_unrelenting(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.UNRELENTING, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=12, upgraded=upgraded, instance_id=_get_next_id())
...
def make_uppercut(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.UPPERCUT, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.UNCOMMON, base_damage=13, effect_vars={'power': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_vicious(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.VICIOUS_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.UNCOMMON, effect_vars={'cards': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_whirlwind(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.WHIRLWIND, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.UNCOMMON, base_damage=5, upgraded=upgraded, has_energy_cost_x=True, instance_id=_get_next_id())
...
def make_aggression(upgraded: bool=False) -> CardInstance:
    kw = frozenset({'innate'}) if upgraded else frozenset()
    return CardInstance(card_id=CardId.AGGRESSION_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=kw, upgraded=upgraded, instance_id=_get_next_id())
...
def make_barricade(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BARRICADE_CARD, cost=3, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, instance_id=_get_next_id())
...
def make_brand(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BRAND, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'hp_loss': 1, 'strength': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_cascade(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CASCADE, cost=-1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, has_energy_cost_x=True, instance_id=_get_next_id())
...
def make_colossus(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.COLOSSUS_CARD, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, base_block=5, effect_vars={'colossus': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_conflagration(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CONFLAGRATION, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.RARE, base_damage=8, effect_vars={'calc_base': 8, 'extra_damage': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_crimson_mantle(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CRIMSON_MANTLE, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'crimson_mantle_power': 8}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_cruelty(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CRUELTY_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'cruelty_power': 25}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_dark_embrace(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DARK_EMBRACE_CARD, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, instance_id=_get_next_id())
...
def make_demon_form(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.DEMON_FORM_CARD, cost=3, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'strength': 2}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_feed(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FEED, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=10, keywords=frozenset({'exhaust'}), can_be_generated_in_combat=False, effect_vars={'max_hp': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_fiend_fire(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.FIEND_FIRE, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=7, keywords=frozenset({'exhaust'}), upgraded=upgraded, instance_id=_get_next_id())
...
def make_hellraiser(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.HELLRAISER_CARD, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, instance_id=_get_next_id())
...
def make_impervious(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.IMPERVIOUS, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, base_block=30, keywords=frozenset({'exhaust'}), upgraded=upgraded, instance_id=_get_next_id())
...
def make_juggernaut(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.JUGGERNAUT_CARD, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'juggernaut_power': 5}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_mangle(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.MANGLE, cost=3, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=15, effect_vars={'strength_loss': 10}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_not_yet(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.NOT_YET, cost=2, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=frozenset({'exhaust'}), effect_vars={'heal': 10}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_offering(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.OFFERING, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, keywords=frozenset({'exhaust'}), effect_vars={'cards': 3, 'energy': 2, 'hp_loss': 6}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_one_two_punch(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.ONE_TWO_PUNCH_CARD, cost=1, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'attacks': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_pacts_end(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PACTS_END, cost=0, card_type=CardType.ATTACK, target_type=TargetType.ALL_ENEMIES, rarity=CardRarity.RARE, base_damage=17, effect_vars={'cards': 3}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_primal_force(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PRIMAL_FORCE, cost=0, card_type=CardType.SKILL, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, instance_id=_get_next_id())
...
def make_pyre(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.PYRE, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, effect_vars={'energy': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_tank(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TANK_CARD, cost=1, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, instance_id=_get_next_id())
...
def make_tear_asunder(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.TEAR_ASUNDER, cost=2, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=5, effect_vars={'repeat': 1, 'calc_base': 0, 'calc_extra': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_thrash(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.THRASH, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.RARE, base_damage=4, upgraded=upgraded, instance_id=_get_next_id())
...
def make_unmovable(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.UNMOVABLE, cost=2, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.RARE, upgraded=upgraded, instance_id=_get_next_id())
...
def make_break(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.BREAK, cost=1, card_type=CardType.ATTACK, target_type=TargetType.ANY_ENEMY, rarity=CardRarity.ANCIENT, base_damage=20, effect_vars={'vulnerable': 5}, upgraded=upgraded, instance_id=_get_next_id())
...
def make_corruption(upgraded: bool=False) -> CardInstance:
    return CardInstance(card_id=CardId.CORRUPTION_CARD, cost=3, card_type=CardType.POWER, target_type=TargetType.SELF, rarity=CardRarity.ANCIENT, effect_vars={'power': 1}, upgraded=upgraded, instance_id=_get_next_id())
...
def create_ironclad_starter_deck() -> list[CardInstance]:
    ...
    return deck
IRONCLAD_CARD_POOL = {CardRarity.BASIC: [make_strike_ironclad, make_defend_ironclad, make_bash], CardRarity.COMMON: [make_anger, make_armaments, make_blood_wall, make_bloodletting, make_body_slam, make_breakthrough, make_cinder, make_havoc, make_headbutt, make_iron_wave, make_molten_fist, make_perfected_strike, make_pommel_strike, make_setup_strike, make_shrug_it_off, make_sword_boomerang, make_thunderclap, make_tremble, make_true_grit, make_twin_strike], CardRarity.UNCOMMON: [make_ashen_strike, make_battle_trance, make_bludgeon, make_bully, make_burning_pact, make_demonic_shield, make_dismantle, make_dominate, make_drum_of_battle, make_evil_eye, make_expect_a_fight, make_feel_no_pain, make_fight_me, make_flame_barrier, make_forgotten_ritual, make_grapple, make_hemokinesis, make_howl_from_beyond, make_infernal_blade, make_inferno, make_inflame, make_juggling, make_pillage, make_rage, make_rampage, make_rupture, make_second_wind, make_spite, make_stampede, make_stomp, make_stone_armor, make_taunt, make_unrelenting, make_uppercut, make_vicious, make_whirlwind], CardRarity.RARE: [make_aggression, make_barricade, make_brand, make_cascade, make_colossus, make_conflagration, make_crimson_mantle, make_cruelty, make_dark_embrace, make_demon_form, make_feed, make_fiend_fire, make_hellraiser, make_impervious, make_juggernaut, make_mangle, make_not_yet, make_offering, make_one_two_punch, make_pacts_end, make_primal_force, make_pyre, make_stoke, make_tank, make_tear_asunder, make_thrash, make_unmovable], CardRarity.ANCIENT: [make_break, make_corruption]}
