...
| Test functions                | 5,387 (~140 test files)                           |
...
| Combat observation (obs v4)   | 294 dimensions                                    |
...
### Self-Attention Observation Space (obs v4)

Combat observations are a **294-dimensional** `float32` vector encoded by [`sts2_env/gym_env/observation.py`](sts2_env/gym_env/observation.py). Attention and GNN policies parse this flat vector into **48 entity tokens** via [`sts2_env/training/entity_tokens.py`](sts2_env/training/entity_tokens.py): player, piles, character mechanics, 10 hand cards, 5 enemies, and 30 relic slots.
...
| `mlp` | SB3 default | Flat MLP on 294-dim vector (baseline) |

**Checkpoint compatibility:** `mlp`, `attention`, and `gnn` checkpoints are **not interchangeable**. Obs v3 (268-dim) and earlier checkpoints are obsolete — retrain on obs v4.

### Draw Pile Memory

Dims 63–88 within the 32-dim pile block are encoded by [`sts2_env/gym_env/pile_distribution.py`](sts2_env/gym_env/pile_distribution.py):
...
| Known draw order | 5 | Type encoding of top 5 draw pile cards |
...
Plus 3 count features (draw/discard/exhaust /20) and 3 reserved padding = **32 pile dims total**. Always enabled in obs v4; the bridge mod must serialize `draw_pile`, `discard_pile`, and `play_pile` card arrays for live-game parity (see [docs/KNOWN_ISSUES.md](docs/KNOWN_ISSUES.md)).
...
| ------ | ------------- |
| Vulnerable on enemy (player-applied) | +0.02 per stack |
| Weak on enemy (player-applied) | +0.02 per stack |
| Block vs enemy attack | +0.001 per HP blocked |

**Run-level shaping:** floor progress (+0.05/floor), combat clear (+0.10), same non-linear HP penalty on macro transitions.
...
Combat is always delegated to a **frozen** combat MaskablePPO (actions 0–114). The Navigator never sees the combat action slice during fights. Strategic observations are encoded separately in [`navigator_observation.py`](sts2_env/gym_env/navigator_observation.py) (164 dims).
...
1. Root = current combat snapshot (`deepcopy`).
2. Each simulation: PUCT child selection, expand with masked PPO action priors, roll out with `apply_combat_action()` until the player turn ends or `--mcts-max-depth` is reached.
3. Leaf value from PPO critic (`predict_combat_values()`).
4. Pick highest visit-count root child (temperature 0).

Search is **bounded to the current player turn** — it does not plan across enemy phases into the next player turn.
...
Staged encounter and deck widening in [`sts2_env/training/combat_curriculum.py`](sts2_env/training/combat_curriculum.py) with HP-retention promotion gates evaluated on a **fixed easy encounter subset** (not gamed by harder training samples).

| Stage | Encounters | Characters | Notes |
| ----- | ---------- | ---------- | ----- |
| `easy_pair` | Jaw Worm + Cultists | Ironclad, Regent | Gate: 98% win, 92% HP |
...
| `recovery` | Elites + bosses | Ironclad, Regent | 100% compromised starts; excluded from `--curriculum full` |

Parallel workers sync stage index via `output_dir/curriculum_state.json`. TensorBoard logs `curriculum/gate_win_rate` and `curriculum/gate_hp_ratio`.
...
1. **Combat curriculum** — `--curriculum easy_pair` through `mixed_acts`, or `--curriculum full --auto-promote` (~5M steps). Target: >98% gate win rate, >92% HP retention per stage.
...
|   |   |-- entity_tokens.py       # Shared 48-node tokenization
...
|   |   |-- combat_env.py          # Single-combat env (Discrete(115), obs 294)
|   |   |-- run_env.py             # Full-run env (Discrete(157), obs 314)
|   |   |-- navigator_env.py       # Strategic env (obs 164, combat delegated)
|   |   |-- hierarchical_run_env.py  # Legacy hierarchical wrapper
|   |   |-- observation.py         # CombatState -> 294-dim obs v4
...
|       |-- state_adapter.py       # Game JSON -> 294-dim observation
|       +-- agent_runner.py        # Main agent loop (optional MCTS)
|
|-- bridge_mod/                    # C# Bridge Mod (Godot project)
|-- tests/                         # ~140 test files, 5,387 test functions
...
- **Default policy:** 2-layer masked Transformer (`CombatAttentionExtractor`) over 48 entity tokens; alternatives are DenseGAT (`--policy gnn`) or flat MLP (`--policy mlp`)
...
| ----------- | ----------- | ------------ |
| Combat (`STS2CombatEnv`) | 294-dim obs v4 | Discrete(115) |
| Full run (`STS2RunEnv`) | 314-dim (294 combat + 20 run state) | Discrete(157) |
| Navigator (`STS2NavigatorEnv`) | 164-dim strategic | Discrete(157) meta layout |
...
| Vulnerable on enemy | +0.02/stack |
| Weak on enemy | +0.02/stack |
...
Optional **turn-bounded MCTS** wraps the trained combat PPO at inference time. PPO provides action priors and leaf values; PUCT search selects the action with the highest visit count. Enabled via `--mcts` on `train_combat.py` (post-train eval), `eval_full_run.py`, or `agent_runner.py`.
...
| [docs/CARDS_REFERENCE.md](docs/CARDS_REFERENCE.md) | All 577 cards |
